...
async function selectDate(targateYear:string, targetMonth:string, targetDate:string, page:Page, isFuture:boolean)
{
    while(true)
    {
     const currentMonth =  page.locator('.ui-datepicker-month').textContent()
      const curretnYear =  page.locator('.ui-datepicker-year').textContent()

      if(await currentMonth===targetMonth && await curretnYear===targateYear)
      {
        break;
      }
